...
interface CompensationTabsProps {
    tabs: Tab[];
    activeTab: TabId;
...
export default function CompensationTabs({ 
    tabs, 
    activeTab, 
    stepStatus, 
    stepOrder 
}: CompensationTabsProps) {
    const isTabEnabled = (tabId: TabId) => {
        if (tabId === 'overview') return true;
        if (tabId === 'compensation-structure') return true; // Always enabled
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
            {tabs.map((tab) => {
                ...
                const completed = tab.id === 'overview' 
                    ? stepStatus['compensation-structure'] 
                    : stepStatus[tab.id];
