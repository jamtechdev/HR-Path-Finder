...
const STEPS: StepConfig[] = [
    { id: 'policy-snapshot', name: 'Policy Snapshot', description: "Answer questions about your company's job classification framework.", icon: <Search className="w-5 h-5" />, route: 'policy-snapshot', estMin: 4 },
    ...
    { id: 'finalization', name: 'Finalization', description: 'Review and finalize all job definitions before mapping.', icon: <Settings className="w-5 h-5" />, route: 'finalization', estMin: 2 },
    { id: 'org-chart-mapping', name: 'Org Chart Mapping', description: 'Map finalized jobs to organizational units and reporting lines.', icon: <Building2 className="w-5 h-5" />, route: 'org-chart-mapping', estMin: 3 },
    { id: 'review-submit', name: 'Review & Submit', description: 'Final review and submission of all job analysis data.', icon: <Rocket className="w-5 h-5" />, route: 'review-submit', estMin: 2 },
];
...
export default function JobAnalysisOverview({
    ...
    return (
        <div className="min-h-full flex flex-col bg-[#f5f3ef] text-[#1e293b]">
            {/* Hero Section - matches HTML .hero-section */}
            <section className="bg-[#0f172a] text-white px-6 py-10 pb-20 md:px-[10%]">
                ...
                {/* Before you begin — spec intro */}
                <div className="mt-6 p-5 rounded-xl bg-white/5 border border-white/10 max-w-[720px]">
                    <div className="text-[11px] font-bold uppercase tracking-wider text-[#b38e5d] mb-2">Before you begin</div>
                    <p className="text-[#e2e8f0] text-sm leading-relaxed m-0">
                        This stage is not intended to redesign or change your current organizational structure.
                        ...
                        Your responses will be used solely as baseline inputs for the subsequent design of the performance management and compensation systems.
                        All inputs are confidential and will not be shared with other employees.
                    </p>
                </div>

                {/* Progress Card - matches HTML .progress-card */}
                <div className="mt-8 bg-white/5 border border-white/10 rounded-xl p-6 flex flex-col md:flex-row md:items-center gap-6 md:gap-10">
                    <div className="border-r-0 md:border-r md:border-white/10 pr-0 md:pr-10">
                        ...
                        <strong className="text-2xl text-[#b38e5d]">{completedCount} / {STEPS.length}</strong>
            ...
            {/* Timeline Section - matches HTML .timeline-container */}
                            ...
                            <div key={step.id} className="flex gap-6">
                                ...
                                >
                                    <div className="min-w-0">
                                        ...
                                        <p className={cn('text-sm mt-1 mb-3', enabled ? 'text-[#64748b]' : 'text-[#cbd5e1]')}>
                                            {step.description}
                                        </p>
                                        <div className={cn('text-xs', enabled ? 'text-[#94a3b8]' : 'text-[#cbd5e1]')}>
                                            ~{step.estMin} min
            ...
            {/* Footer - sticky inside main content (sidebar layout), full width of content area */}
