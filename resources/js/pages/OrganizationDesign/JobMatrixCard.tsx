...
export default function JobMatrixCard({ project, jobDefinition, orgMapping, reportingHierarchy }: Props) {
    ...
    useEffect(() => {
        try {
            const saved = localStorage.getItem('jobMatrixComfortMode');
            if (saved === '1') setComfortMode(true);
        } catch (_) {}
    }, []);
    ...
    const toggleComfortMode = () => {
        setComfortMode((prev) => {
            const next = !prev;
            try {
                localStorage.setItem('jobMatrixComfortMode', next ? '1' : '0');
            } catch (_) {}
            return next;
        });
    };
    ...
    return (
        <AppLayout>
            ...
            />
            <div
                className={`mx-auto ${comfortMode ? 'max-w-[1650px] p-8 md:p-10' : 'max-w-[1500px] p-6 md:p-8'}`}
            >
                ...
                <div className="mb-6">
                    <div className="mb-3 flex justify-end">
                        ...
                        >
                            {comfortMode ? 'Comfort View: ON' : 'Comfort View: OFF'}
                        </button>
                    </div>
                    <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-7 rounded-t-lg shadow-lg">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                                <h1 className={comfortMode ? 'text-5xl font-bold' : 'text-4xl font-bold'}>Job Matrix Card</h1>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <Card className="border-t-0 rounded-t-none shadow-xl">
                    <CardHeader className="bg-gradient-to-r from-yellow-50 via-yellow-100/50 to-yellow-50 border-b-4 border-yellow-200">
                        ...
                        <div className={`grid grid-cols-12 ${comfortMode ? 'gap-5' : 'gap-4'}`}>
                            <div className="col-span-2 bg-yellow-200 p-3.5 rounded-lg border-2 border-yellow-300">
                                <p className="text-sm font-bold text-gray-700 uppercase mb-1">Job Group</p>
                                <p className="font-bold text-lg">{jobDefinition.job_keyword?.category || '경영'}</p>
                            </div>
                            <div className="col-span-3 bg-gray-100 p-3.5 rounded-lg border-2 border-gray-300">
                                <p className="text-sm font-bold text-gray-700 uppercase mb-1">Job Name</p>
                                <p className="font-bold text-lg">{jobDefinition.job_name}</p>
                            </div>
                            <div className="col-span-7 bg-white p-3.5 rounded-lg border-2 border-gray-200">
                                <p className="text-sm font-bold text-gray-700 uppercase mb-1">Job Purpose</p>
                                <p className={comfortMode ? 'text-lg leading-relaxed' : 'text-base leading-relaxed'}>{jobPurpose || jobDefinition.job_description || 'Not specified'}</p>
                            </div>
                        </div>
                    </CardHeader>

                    <CardContent className={`${comfortMode ? 'p-8 space-y-8' : 'p-6 space-y-6'} bg-gradient-to-br from-white to-gray-50/50`}>
                        {/* Personnel Information */}
                        <div className="grid grid-cols-12 gap-4">
                            <div className="col-span-3">
                                <p className="text-sm font-bold text-gray-600 uppercase mb-2">Job Specialist</p>
                                <p className={comfortMode ? 'text-lg font-semibold' : 'text-base font-semibold'}>
                                    {jobSpecialist 
                            ...
                            <div className="col-span-3">
                                <p className="text-sm font-bold text-gray-600 uppercase mb-2">Executive Director</p>
                                <p className={comfortMode ? 'text-lg font-semibold' : 'text-base font-semibold'}>
                                    {orgMapping?.org_head?.name 
                                        ? `${orgMapping.org_head.name} (${orgMapping.org_head.rank}, ${orgMapping.org_head.title})`
                                        : 'Not assigned'
                            ...
                            <div className="col-span-2">
                                <p className="text-sm font-bold text-gray-600 uppercase mb-2">Job Code</p>
                                <p className={comfortMode ? 'text-lg' : 'text-base'}>직무코드</p>
                            </div>
                            <div className="col-span-4">
                                <p className="text-sm font-bold text-gray-600 uppercase mb-2">Reporting Hierarchy</p>
                                <p className={comfortMode ? 'text-lg font-medium' : 'text-base font-medium'}>{reportingHierarchy || 'Team Leader → Director → CEO'}</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {responsibilities.length > 0 ? (
                                <div className={`grid grid-cols-2 md:grid-cols-3 ${comfortMode ? 'gap-4' : 'gap-3'}`}>
                                    {responsibilities.map((resp, idx) => (
                                        <div key={idx} className="bg-blue-50 border-2 border-blue-200 rounded-lg p-3 hover:bg-blue-100 transition-colors">
                                            <p className={comfortMode ? 'text-lg font-medium' : 'text-base font-medium'}>{resp}</p>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                    ...
                                    {['HR policy planning and improvement', 'Management & compliance with labor laws', 
                                      ...
                                      'Compensation administration', 'HR-ERP operation & enhancement'].map((resp, idx) => (
                                        <div key={idx} className="bg-gray-50 border-2 border-gray-200 rounded-lg p-3">
                                            <p className={comfortMode ? 'text-lg text-muted-foreground' : 'text-base text-muted-foreground'}>{resp}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
                        ...
                        <div>
                            ...
                            <div className="overflow-x-auto">
                                <table className={`w-full border-collapse border-2 border-gray-300 ${comfortMode ? 'text-[1.05rem]' : ''}`}>
                                    <thead>
                                        <tr className="bg-gray-100">
                                            <th className="border-2 border-gray-300 p-3 text-left text-sm font-bold uppercase">Category</th>
                                            <th className="border-2 border-gray-300 p-3 text-left text-sm font-bold uppercase bg-red-50">Required</th>
                                            <th className="border-2 border-gray-300 p-3 text-left text-sm font-bold uppercase bg-green-50">Preferred</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td className="border-2 border-gray-300 p-3 font-semibold text-base">Education</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-red-50">{spec.education?.required || '-'}</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-green-50">{spec.education?.preferred || '-'}</td>
                                        </tr>
                                        <tr>
                                            <td className="border-2 border-gray-300 p-3 font-semibold text-base">Major</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-red-50">{spec.major?.required || spec.experience?.required || '-'}</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-green-50">{spec.major?.preferred || spec.experience?.preferred || '-'}</td>
                                        </tr>
                                        <tr>
                                            <td className="border-2 border-gray-300 p-3 font-semibold text-base">Certification</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-red-50">{spec.certification?.required || '-'}</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-green-50">{spec.certification?.preferred || '-'}</td>
                                        </tr>
                                        <tr>
                                            <td className="border-2 border-gray-300 p-3 font-semibold text-base">Tech. Skills</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-red-50">{spec.skills?.required || '-'}</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-green-50">{spec.skills?.preferred || '-'}</td>
                                        </tr>
                                        <tr>
                                            <td className="border-2 border-gray-300 p-3 font-semibold text-base">Language</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-red-50">{spec.language?.required || '-'}</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-green-50">{spec.language?.preferred || '-'}</td>
                                        </tr>
                                        <tr>
                                            <td className="border-2 border-gray-300 p-3 font-semibold text-base">Other</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-red-50">{spec.other?.required || '-'}</td>
                                            <td className="border-2 border-gray-300 p-3 text-base bg-green-50">{spec.other?.preferred || '-'}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                            ...
                            {competencyLevels.length > 0 ? (
                                <div className="overflow-x-auto border-2 border-gray-300 rounded-lg">
                                    <table className="w-full border-collapse">
                                        <thead>
                                            <tr className="bg-gray-100">
                                                <th className="border-2 border-gray-300 p-3 text-left text-sm font-bold uppercase">Level</th>
                                                <th className="border-2 border-gray-300 p-3 text-left text-sm font-bold uppercase">Expected Behavior by Level</th>
                                                <th className="border-2 border-gray-300 p-3 text-left text-sm font-bold uppercase w-36">Dev. Period</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {competencyLevels.map((level, idx) => (
                                                <tr key={idx} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="border-2 border-gray-300 p-3 text-base whitespace-pre-line">
                                                        {level.description}
                                                    </td>
                                                    <td className="border-2 border-gray-300 p-3 text-base font-medium text-center">
                        ...
                        <div>
                            <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4 rounded-t-lg mb-0">
                                <h3 className="text-2xl font-bold">Job Value Assessment Framework Pre-View</h3>
                            </div>
                            <div className="bg-blue-50 border-2 border-blue-200 border-t-0 rounded-b-lg p-6">
                                ...
                                <div className="grid grid-cols-3 gap-4 mb-6">
                                    ...
                                    <p className="text-sm font-bold text-gray-700 uppercase mb-2">Job Complexity</p>
                                        <Badge className="bg-blue-600 text-white text-base px-4 py-1">High</Badge>
                                    </div>
                                    <div className="bg-yellow-200 border-2 border-yellow-400 rounded-lg p-4 text-center">
                                    <p className="text-sm font-bold text-gray-700 uppercase mb-2">Business Impact</p>
                                        <Badge className="bg-blue-600 text-white text-base px-4 py-1">Medium</Badge>
                                    </div>
                                    <div className="bg-yellow-200 border-2 border-yellow-400 rounded-lg p-4 text-center">
                                    <p className="text-sm font-bold text-gray-700 uppercase mb-2">Replacement Difficulty</p>
                                        <Badge className="bg-blue-600 text-white text-base px-4 py-1">Medium</Badge>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-white border-2 border-gray-300 rounded-lg p-3">
                                        <p className="text-sm font-bold text-gray-600 uppercase mb-1">Total Job Grade Points</p>
                                        <p className="text-lg font-bold">3,500 / 9,850</p>
                                    </div>
                                    <div className="bg-white border-2 border-gray-300 rounded-lg p-3">
                                        <p className="text-sm font-bold text-gray-600 uppercase mb-1">Job Rank within Company</p>
                        ...
                        {sortedCsfs.length > 0 && (
                            <div>
                                ...
                                <div className={`grid grid-cols-4 ${comfortMode ? 'gap-3' : 'gap-2'}`}>
                                    ...
                                    {['low', 'medium', 'high'].map(capability => (
                                        <div key={`high-${capability}`} className="min-h-[150px] border-2 border-gray-300 rounded-lg p-3 bg-yellow-50">
                                            {csfMatrix.high[capability as keyof typeof csfMatrix.high].map((csf, idx) => (
                                                <div key={idx} className="mb-2 last:mb-0 bg-yellow-200 border border-yellow-400 rounded p-2.5">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <p className="text-sm font-semibold flex-1">{csf.name}</p>
                                                        {csf.rank && (
                                    ...
                                    {['low', 'medium', 'high'].map(capability => (
                                        <div key={`medium-${capability}`} className="min-h-[150px] border-2 border-gray-300 rounded-lg p-3 bg-gray-50">
                                            {csfMatrix.medium[capability as keyof typeof csfMatrix.medium].map((csf, idx) => (
                                                <div key={idx} className="mb-2 last:mb-0 bg-gray-200 border border-gray-400 rounded p-2.5">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <p className="text-sm font-semibold flex-1">{csf.name}</p>
                                                        {csf.rank && (
                                                            <Badge variant="outline" className="text-xs">Rank {csf.rank}</Badge>
                                    ...
                                    {['low', 'medium', 'high'].map(capability => (
                                        <div key={`low-${capability}`} className="min-h-[150px] border-2 border-gray-300 rounded-lg p-3 bg-red-50">
                                            {csfMatrix.low[capability as keyof typeof csfMatrix.low].map((csf, idx) => (
                                                <div key={idx} className="mb-2 last:mb-0 bg-red-200 border border-red-400 rounded p-2.5">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <p className="text-sm font-semibold flex-1">{csf.name}</p>
                                                        {csf.rank && (
                                                            <Badge variant="outline" className="text-xs">Rank {csf.rank}</Badge>
